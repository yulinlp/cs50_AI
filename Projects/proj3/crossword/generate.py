...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        for variable in self.domains.items():
            ...
            for item in variable[1]:
                if len(item) != length:
                    to_rem.add(item)
            variable[1].difference_update(to_rem)
        ...
        if overlap:
            ...
            for value_x in self.domains[x]:
                ...
                for value_y in self.domains[y]:
                    if value_y[lap_y] == value_x[lap_x] and value_y != value_x:
                        flag = True
                        break
                if not flag:
                    xs_to_rem.add(value_x)
            if xs_to_rem:
                self.domains[x] = self.domains[x].difference(xs_to_rem)
                is_changed = True
        ...
        while arcs:
            ...
            if self.revise(x,y):
                if len(self.domains[x]) == 0:
                    ...
                    for neighbour_x in self.crossword.neighbors(x) - {y}:
                        arcs.appendleft((neighbour_x,x))
    ...
    def order_domain_values(self, var, assignment):
        ...
        for variable in self.domains[var]:
            ...
            for neighbor in neighbors:
                for variable_neighbor in self.domains[neighbor]:
                    ...
                    if overlap:
                        a, b = overlap
                        if variable[a] != variable_neighbor[b]:
                            ruled_out += 1
        ...
        for variable in self.crossword.variables:  # iterate over all variables in the crossword
            if variable not in assignment:  # if the variable is unassigned (meaning it is not in assignment)
                ...
                potential_variables.append(
                    [variable, len(self.domains[variable]), len(self.crossword.neighbors(variable))])
    ...
    def backtrack(self, assignment):
        ...
        # traverse over all values in the domain that it sorted with heuristics (the least constraining values)
